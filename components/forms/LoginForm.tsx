    ...
    <form onSubmit={handleSubmit} className="mt-6 space-y-5">
      ...
      <div className="flex items-center gap-2">
        ...
        <button type="button" onClick={() => fillDemo('admin')}
          className="inline-flex items-center gap-1.5 rounded-full bg-amber-50 border border-amber-200 px-3 py-1 text-xs font-medium text-amber-700 hover:bg-amber-100 transition-colors">
          <span className="material-symbols-rounded text-sm">admin_panel_settings</span> Admin
        </button>
        <button type="button" onClick={() => fillDemo('user')}
          className="inline-flex items-center gap-1.5 rounded-full bg-green-50 border border-green-200 px-3 py-1 text-xs font-medium text-green-700 hover:bg-green-100 transition-colors">
          <span className="material-symbols-rounded text-sm">person</span> Utilisateur
        </button>
      </div>
      ...
      <div>
        ...
        <div className="relative">
          ...
          <input id="password" name="password" type={showPassword ? 'text' : 'password'} required
            ...
            className="w-full rounded-xl border border-gray-200 bg-gray-50 py-3 pl-11 pr-11 text-sm text-gray-900 placeholder-gray-400 focus:border-green-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-green-500/20 transition-all"
            placeholder="••••••••" />
      ...
      <button type="submit" disabled={loading}
        className="w-full rounded-xl bg-green-700 px-4 py-3 text-sm font-semibold text-white shadow-sm hover:bg-green-800 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed transition-colors inline-flex items-center justify-center gap-2">
        {loading ? (
          <><span className="material-symbols-rounded text-lg animate-spin">progress_activity</span> Connexion en cours...</>
        ) : (
